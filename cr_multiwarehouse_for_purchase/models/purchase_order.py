...
class PurchaseOrderInherit(models.Model):
    ...
    def button_approve(self, force=False):
        ...
        if self.company_id.cr_allow_purchase_warehouse:
            self.create_picking()
        return result
    ...
    def create_picking(self):
        ...
        for order in self.filtered(lambda po: po.state in ('purchase', 'done')):
            ...
            if order_lines:
                ...
                warehouse_ids = []
                if not pickings:
                    [warehouse_ids.append(x.warehouses_id) for x in order.order_line if x.warehouses_id not in warehouse_ids]
                    for warehouse_id in warehouse_ids:
                        po_lines = self.env['purchase.order.line'].search([('warehouses_id', '=', warehouse_id.id), ('order_id', '=', self.id)])
                        res = order.prepare_picking(po_lines[0])
                        picking = StockPicking.create(res)
                        picking_ids += picking
                else:
                    picking_ids = pickings[0]
                lines = []
                for warehouse_id in warehouse_ids:    
                    po_lines = self.env['purchase.order.line'].search([('warehouses_id', '=', warehouse_id.id), ('order_id', '=', self.id)])    
                    for i in picking_ids:
                        if i.location_dest_id.warehouse_id.id == warehouse_id.id:
